...
@dataclass
class PhoenixRoundLogger:
    """Writes per-round PHOENIX records from History.extras['phoenix_rounds']."""
    ...
    def open(self, *, config: Any) -> None:
        ...
        self._f = (self.run_dir / "phoenix_rounds.jsonl").open("a", encoding="utf-8")
        self._written = 0

    def log_unit(self, *, unit_id: int, history: Any, metrics: dict[str, Any]) -> None:
        del unit_id, metrics
        if self._f is None:
            raise RuntimeError("PhoenixRoundLogger not opened.")
        extras = getattr(history, "extras", None) or {}
        rounds = list(extras.get("phoenix_rounds", []))
